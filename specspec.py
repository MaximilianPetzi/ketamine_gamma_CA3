#plots data from seedavg.py simulation if done as spectrum
import seedavg 
withspec=seedavg.withspec
import numpy as np
...
DatShape=np.shape(Data)[0],np.shape(Data)[1],np.shape(Data)[2],np.shape(Data)[3],np.shape(Data)[4],2,len(Data[0,0,0,0,0][0])#last value len(...) is spectrum array length
        ...
        for c in range(len(Data[0,0])):
            for d in range(len(Data[0,0,0])):
                for e in range(len(Data[0,0,0,0])):     #build proper Tensor
                    Dat[a,b,c,d,e]=Data[a,b,c,d,e][:2]      #full spectrum and freqs   
...
print("shapes now")
print(np.shape(Data[0,0,0,0,0]))
print(np.shape(dat))
print("end")

def bandpower(i,start, end):#give function the specific frequency array you want
    #with this, in the case of a withspec simulation, you can calculate bandpower of condition i averaged over all seeds
    specs=np.average(dat[:,:,:,:,:,1,:],axis=1)
    p=specs[0,0,i,0]
    f=dat[0,0,0,0,0,0]
    bpow=0
...
def avgspec():
    ...
    x=dat[0,0,0,0,0,0] #f
    fig, ax = plt.subplots(nrows=DatShape[3], ncols=1, figsize=(10, 10))
    for k in range(DatShape[3]):#choose dimension as simulated over
        ax[k].plot(x,specs[0,0,k,0],label="k="+str(k))
        ax[k].set_xlim([0,100])
        ax[k].set_ylim([0,5])
    fig.legend()
    plt.show()
