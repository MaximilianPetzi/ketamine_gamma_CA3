...
#define Expo Expo_ 
#define _f_Expo _f_Expo_ 
#define fspitchar fspitchar_ 
...
extern "C" {
 ...
 static void _hoc_Expo(void);
 static void _hoc_MAXxy(void);
 static void _hoc_MINxy(void);
 static void _hoc_Str2Num(void);
 static void _hoc_SUMEXP(void);
 static void _hoc_byteswap(void);
 static void _hoc_ceilg(void);
 static void _hoc_eqojt(void);
 static void _hoc_fgchar(void);
 static void _hoc_fspitchar(void);
 static void _hoc_flor(void);
 static void _hoc_isojn(void);
 static void _hoc_isojt(void);
 static void _hoc_install_vecst(void);
 static void _hoc_mkcodf(void);
 static void _hoc_ojtnum(void);
 static void _hoc_recodf(void);
 static void _hoc_uncodf(void);
 static void _hoc_vlsz(void);
...
extern Memb_func* memb_func;
 ...
 "MAXxy", _hoc_MAXxy,
 "MINxy", _hoc_MINxy,
 "Str2Num", _hoc_Str2Num,
 "SUMEXP", _hoc_SUMEXP,
 "byteswap", _hoc_byteswap,
 "ceilg", _hoc_ceilg,
 "eqojt", _hoc_eqojt,
 "fgchar", _hoc_fgchar,
 "fspitchar", _hoc_fspitchar,
 "flor", _hoc_flor,
 "isojn", _hoc_isojn,
 "isojt", _hoc_isojt,
 ...
 extern double MAXxy( );
 extern double MINxy( );
 extern double Str2Num( );
 extern double SUMEXP( );
 extern double byteswap( );
 extern double ceilg( );
 extern double eqojt( );
 extern double fgchar( );
 extern double flor( );
 extern double isojn( );
 extern double isojt( );
 extern double mkcodf( );
 extern double ojtnum( );
 extern double recodf( );
 extern double uncodf( );
 extern double vlsz( );
 /* declare global and static user variables */
 double ALL = -1.3479e+120;
 double BVBASE = 0;
 double CHK = -1.3476e+120;
 double DEBUG_VECST = 0;
 double EBE = -1.346e+120;
 double EBI = -1.3462e+120;
 double EQY = -1.34665e+120;
 double EQX = -1.3467e+120;
 double EQW = -1.3468e+120;
 double EQV = -1.3469e+120;
 double EQU = -1.347e+120;
 double ERR = -1.348e+121;
 double GTE = -1.3473e+120;
 double GTH = -1.3474e+120;
 double GET = -1.3479e+121;
 double IBI = -1.3461e+120;
 double IBE = -1.3463e+120;
 double INTERP_VECST = 1;
 double LTE = -1.3471e+120;
 double LTH = -1.3472e+120;
 double LOOSE = 1e-06;
 double NEQ = -1.3466e+120;
 double NOZ = -1.3475e+120;
 double NEG = -1.3478e+120;
 double NOP = -1.3476e+121;
 double OK = -1.3477e+121;
 double POS = -1.3477e+120;
 double RXP = -1.3464e+120;
 double RES = 0;
 double SEQ = -1.3465e+120;
 double SET = -1.3478e+121;
 double VERBOSE_VECST = 1;
 double VECST_INSTALLED = 0;
 double usetable = 1;
 ...
 "BVBASE", &BVBASE,
 "VECST_INSTALLED", &VECST_INSTALLED,
 "DEBUG_VECST", &DEBUG_VECST,
 "VERBOSE_VECST", &VERBOSE_VECST,
 "INTERP_VECST", &INTERP_VECST,
 "LOOSE", &LOOSE,
 "ERR", &ERR,
 "GET", &GET,
 "SET", &SET,
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 void _vecst_reg() {
  ...
  _initlists();
 	...
 	ivoc_help("help ?1 vecst /home/maximilian/Desktop/work/neymotin/139421-master/vecst.mod\n");
 }
...
double EXP (  double _lx ) {
   double _lEXP;
 if ( _lx > 20.0  || _lx < - 20.0 ) {
     printf ( "EXP(%g) called with OOB value [-20,20]\n" , _lx ) ;
     _lEXP = ERR ;
     }
   else {
...
static int  fspitchar (  double _lc ) {
