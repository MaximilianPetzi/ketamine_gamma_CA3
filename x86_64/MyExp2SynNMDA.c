...
#define nrn_init _nrn_init__MyExp2SynNMDA
#define _nrn_initial _nrn_initial__MyExp2SynNMDA
#define nrn_cur _nrn_cur__MyExp2SynNMDA
#define _nrn_current _nrn_current__MyExp2SynNMDA
#define nrn_jacob _nrn_jacob__MyExp2SynNMDA
#define nrn_state _nrn_state__MyExp2SynNMDA
#define _net_receive _net_receive__MyExp2SynNMDA 
#define state state__MyExp2SynNMDA 
...
#define A2 _p[14]
#define B2 _p[15]
#define mgblock _p[16]
#define factor _p[17]
#define factor2 _p[18]
#define DA _p[19]
#define DB _p[20]
#define DA2 _p[21]
#define DB2 _p[22]
...
#define mg mg_MyExp2SynNMDA
 ...
 "mg_MyExp2SynNMDA", &mg_MyExp2SynNMDA,
 0,0
};
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 pnt_receive_size[_mechtype] = 2;
 	...
 	ivoc_help("help ?1 MyExp2SynNMDA /home/maximilian/Desktop/work/neymotin/139421-master/MyExp2SynNMDA.mod\n");
...
{  double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt;
 ...
 B = B + factor * _args[0] ;
   ...
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = A2;
    double __primary = (A2 + factor2 * _args[0] * r) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau1NMDA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau1NMDA ) - __primary );
    A2 += __primary;
  } else {
 A2 = A2 + factor2 * _args[0] * r ;
       }
   if (nrn_netrec_state_adjust && !cvode_active_){
    /* discon state adjustment for cnexp case (rate uses no local variable) */
    double __state = B2;
    double __primary = (B2 + factor2 * _args[0] * r) - __state;
     __primary += ( 1. - exp( 0.5*dt*( ( - 1.0 ) / tau2NMDA ) ) )*( - ( 0.0 ) / ( ( - 1.0 ) / tau2NMDA ) - __primary );
    B2 += __primary;
  } else {
 B2 = B2 + factor2 * _args[0] * r ;
       }
 }
   else {
     if ( r > - 1000.0 ) {
...
#if NMODL_TEXT
static const char* nmodl_filename = "/home/maximilian/Desktop/work/neymotin/139421-master/MyExp2SynNMDA.mod";
static const char* nmodl_file_text = 
